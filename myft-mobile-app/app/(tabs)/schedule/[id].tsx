...
export default function GameDetail() {
  ...
  const found = useMemo(() => {
    for (const day of scheduleData) {
      ...
      if (game) return { dayLabel: day.label, game };
    }
    return null;
  }, [id]);
  ...
  const cap1Last = t1?.captain ? t1.captain.split(' ').slice(-1)[0] : '';
  const cap2Last = t2?.captain ? t2.captain.split(' ').slice(-1)[0] : '';
  ...
  const activeName = side === 'team1' ? name1 : name2;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.toggleRow}>
        ...
        >
          <Text style={[styles.toggleText, side === 'team1' && styles.toggleTextActive]}>{name1}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setSide('team2')}
          style={[styles.toggleBtn, side === 'team2' && styles.toggleActive]}
        >
          <Text style={[styles.toggleText, side === 'team2' && styles.toggleTextActive]}>{name2}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.tableCard}>
        ...
        <View style={[styles.row, styles.headRow]}>
          ...
          <Text style={[styles.hCell, styles.pNum]}>MVP</Text>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.row}>
              ...
              <Text style={[styles.cell, styles.pNum]}>{item.mvp}</Text>
            </View>
          )}
        />
      </View>
