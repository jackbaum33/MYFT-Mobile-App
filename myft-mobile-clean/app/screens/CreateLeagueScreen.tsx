// screens/CreateLeagueScreen.tsx - form to create a new fantasy league (draft-based)
...
import { createLeague, type DraftStyle } from '../../services/leagues';
...
type NavProp = NavigationProp<FantasyStackParamList, 'CreateLeague'>;

function roundToNext15Min(d: Date): Date {
  const ms = 15 * 60 * 1000;
...
export default function CreateLeagueScreen() {
  ...
  const [selectedUids, setSelectedUids] = useState<Set<string>>(new Set(user?.uid ? [user.uid] : []));
  ...
  const [scheduledStart, setScheduledStart] = useState(() => roundToNext15Min(new Date(Date.now() + 60 * 60 * 1000)));
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        console.warn('[CreateLeague] listUsers failed:', e);
      } finally {
        setUsersLoaded(true);
      }
    })();
  ...
  const toggleMember = (uid: string) => {
    if (uid === user?.uid) return; // owner is always in
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      console.warn('[CreateLeague] createLeague failed:', e);
      Alert.alert('Failed to create league', e?.message ?? 'Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <FlatList
      ...
      renderItem={({ item }) => {
        ...
        return (
          ...
          >
            <View style={{ flex: 1 }}>
              <Text style={styles.userName}>{item.displayName}{isOwner ? ' (you)' : ''}</Text>
      ...
      ListFooterComponent={
        <TouchableOpacity
          ...
          disabled={submitting}
        >
          <Text style={styles.submitBtnText}>{submitting ? 'Creating…' : 'Create League'}</Text>
        </TouchableOpacity>
      }
    />
  );
